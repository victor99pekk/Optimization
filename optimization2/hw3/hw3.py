...
def gradient(weights:np.ndarray[float], batch_size:int=batch_size) -> np.ndarray[float]:
...
def sgd(save_fig:bool=False, nbr_iterations:int = 2000) -> None:
    ...
    if save_fig:
        ...
        plt.legend()
        plt.savefig('sgd_optimization_plot1.png')
        plt.show()
    print(function_list[-1])
    return weights

w = sgd(save_fig=True, nbr_iterations=2000)
print(classify(w))
